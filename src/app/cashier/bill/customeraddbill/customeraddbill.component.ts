...
import { BillCustomerToAdd } from 'src/app/interfaces/bill';
...
@Component({
  selector: 'app-customeraddbill',
  templateUrl: './customeraddbill.component.html',
  styleUrls: ['./customeraddbill.component.scss'],
  ...
  this.loadArticles()
    ...
    this.clientService.getCustomers().subscribe({
      next: (response) => {
        ...
        console.log('Suppliers loaded:', this.clients);
      },
      error: (err) => {
...
loadArticles() {
  ...
  onSubmit() {
    ...
    const billToAdd: BillCustomerToAdd = {
    ...
    this.billCustomerService.addBill(billToAdd).subscribe({
      next: (response) => {
        if (response.success) {
          console.log("Api Response", response)
          ...
          this.billForm.reset();
          this.lineItems = [];
          this.billCustomerService.notifyBillUpdated();
                  this.router.navigate(['/bill/customermanagebill']);
