...
export class BilldetailspopupComponent implements OnInit {
  ...
  bill_id: string= '';
  bill : any = { _id: '', fournisseur: '', type: 'invoice', dateFacturation: '05/20/2025', status: 'en attente', lignes : [], totalHT : 0, tvaRate: 0, totalTTC: 0, numero: ''};
  ...
  this.billService.getBillById(this.bill_id).subscribe({
      next: (response) => {
          this.bill = response;
          console.log("la  facture est : ", this.bill);
